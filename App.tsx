...
export default function App() {
  ...
  // 修改：仅在 currentUser 存在时写入，只有明确注销时由 logout 函数处理清除
  useEffect(() => {
    if (currentUser) {
  ...
  // 修改：使用 upsert 替代 insert，防止 ID 重复导致保存失败
  ...
  const renderContent = () => {
    switch (currentView) {
      case 'tasks':
        return <TaskBoard tasks={tasks} onAddTask={handleAddTask} onUpdateTask={handleUpdateTask} onDeleteTask={handleDeleteTask} currentUser={currentUser} statuses={taskStatuses} onUpdateStatuses={(s) => setTaskStatuses(s)} />;
      case 'finance':
        return <FinanceTracker transactions={transactions} onAddTransaction={handleAddTransaction} onDeleteTransaction={handleDeleteTransaction} tasks={tasks} categories={financeCategories} onUpdateCategories={(c) => setFinanceCategories(c)} />;
      case 'prompts':
  ...
  const renderAdminPanel = () => (
      <div className="bg-slate-900 p-8 rounded-2xl border border-slate-800 shadow-2xl max-w-2xl w-full relative z-10 pointer-events-auto max-h-[90vh] overflow-y-auto">
          <div className="flex justify-between items-center mb-6">
            ...
            <button onClick={() => setShowAdminPanel(false)} className="text-slate-400 hover:text-white">关闭</button>
          </div>
          {!isAdminAuthenticated ? (
              <div className="space-y-4">
                  <input type="password" className="w-full bg-slate-800 border border-slate-700 rounded-lg px-4 py-3 text-white" placeholder="管理员密码" value={adminAuthInput} onChange={(e) => setAdminAuthInput(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && handleAdminLogin()} />
                          ...
                          {users.map(user => (
                              <div key={user.username} className="bg-slate-800 p-4 rounded-lg flex items-center justify-between border border-slate-700">
                                  <div>
                                      ...
                                      <div className="text-xs text-slate-500">状态: {user.isApproved ? <span className="text-green-400">已开通</span> : <span className="text-yellow-400">待审核</span>}</div>
                                  </div>
              ...
              {!showAdminPanel ? (
                  <div className="bg-slate-900 p-8 rounded-2xl border border-slate-800 shadow-2xl max-w-md w-full relative z-10">
                    ...
                    <form onSubmit={authMode === 'login' ? handleLogin : handleRegister} className="space-y-4">
                        ...
                        <button type="submit" className="w-full bg-blue-600 text-white font-bold py-3 rounded-lg">{authMode === 'login' ? '登 录' : '注 册'}</button>
                    </form>
                    <div className="mt-6 text-center">
                        <button onClick={() => setShowAdminPanel(true)} className="text-slate-600 text-xs hover:text-slate-400">管理员入口</button>
                    </div>
                  </div>
              ) : renderAdminPanel()}
          </div>
